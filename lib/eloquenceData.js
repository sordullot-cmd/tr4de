...
/* ─────────────── Défis (ateliers guidés) ───────────────
 * Exercices ciblés inspirés des méthodes de coaches en éloquence. Chaque défi a
 * un `kind` qui pilote son rendu dans l'onglet « Défis » :
 *   - "record"  : enregistrement + analyse IA (réutilise le moteur commun).
 *   - "mirror"  : webcam en miroir + minuteur, sans analyse.
 * Champs de `record` :
 *   - analysisMode : mode envoyé à l'IA ("reading" | "freeSpeech").
 *   - input        : "topic" (champ sujet) | "reference" (texte à relire) | "none".
 *   - durations    : contraintes de temps successives en secondes (ex. résumé).
 *   - forbidden    : propose une liste de mots à bannir (comptés localement).
 *   - targetWpmMax : débit maximal conseillé (affiché).
 *   - timerTargetSec : durée cible affichée par un minuteur (non bloquant).
 *   - drillGoal    : consigne transmise à l'IA pour orienter le jugement.
 * Les sessions issues des défis sont enregistrées sous le mode "drills" (progression
 * groupée), quel que soit l'analysisMode utilisé pour l'IA. */
export const DRILLS = [
  {
    id: "storytelling",
    emoji: "✨",
    title: "Storytelling du jour",
    tagline: "Transforme une anecdote banale en aventure.",
    kind: "record",
    analysisMode: "freeSpeech",
    input: "topic",
    topicPlaceholder: "Ton anecdote du jour : « j'ai acheté du pain »…",
    frameworkId: "raconter",
    description:
      "Chaque jour, raconte une anecdote — même nulle. « Aujourd'hui, je suis allé acheter du pain » peut devenir une aventure. Ajoute du suspense, des détails, des émotions, des dialogues.",
    tips: [
      "Commence par planter le décor, puis fais monter une petite tension.",
      "Glisse au moins un dialogue (« Et là, le boulanger me dit… »).",
      "Termine sur une chute ou une leçon, même minuscule.",
    ],
    drillGoal:
      "Exercice de STORYTELLING : l'utilisateur transforme une anecdote banale en récit vivant. Juge avant tout `structure` (arc narratif : situation → tension → moment fort → chute) et la capacité à créer du relief (détails sensoriels, émotions, dialogues, suspense). Récompense l'engagement narratif ; pénalise un récit plat, purement factuel ou sans progression.",
  },
  {
    id: "debit",
    emoji: "🐢",
    title: "Débit maîtrisé",
    tagline: "Parle 50 % moins vite que d'habitude.",
    kind: "record",
    analysisMode: "freeSpeech",
    input: "topic",
    topicPlaceholder: "Choisis un sujet à développer lentement…",
    targetWpmMax: 110,
    description:
      "Choisis un sujet et parle 50 % moins vite que d'habitude. Pose chaque mot, respire, laisse des silences. Le but n'est pas le contenu, mais le tempo.",
    tips: [
      "Vise un débit bien en dessous de 110 mots/minute.",
      "Autorise-toi de vrais silences entre les idées.",
      "Exagère : ce qui te semble lent est souvent juste normal.",
    ],
    drillGoal:
      "Exercice de DÉBIT LENT : l'utilisateur doit parler nettement plus lentement que la normale (environ moitié moins vite), en posant sa voix. Récompense un débit lent et posé (idéalement sous 110 mots/minute) avec des silences maîtrisés ; pénalise fortement `rhythm` si le débit reste rapide ou précipité. Le fond compte peu, c'est le tempo qui est évalué.",
  },
  {
    id: "no-search",
    emoji: "🔥",
    title: "Ne plus chercher ses mots",
    ...
    timerTargetSec: 180,
    description:
      "Choisis un objet, puis parle dessus pendant trois minutes sans jamais t'arrêter. Peu importe ce que tu dis : l'objectif est de ne laisser aucun blanc et de dompter l'hésitation.",
    tips: [
      ...
      "Vise zéro « euh » : mieux vaut un silence assumé qu'un tic.",
    ],
    drillGoal:
      "Exercice de FLUIDITÉ (parler ~3 minutes sans s'arrêter sur un objet imposé). L'objectif est la continuité : juge principalement `confidence` et `rhythm` sur l'absence d'hésitations, de blancs et de tics de langage. Pénalise les longues pauses et les « euh » ; la profondeur du contenu importe peu ici.",
  },
  {
    id: "summary",
    emoji: "⏱️",
    title: "Le résumé express",
    tagline: "1 minute, puis 30 s, puis 10 s.",
    kind: "record",
    analysisMode: "freeSpeech",
    input: "topic",
    topicPlaceholder: "Le sujet ou la vidéo à résumer…",
    durations: [60, 30, 10],
    description:
      "Regarde une vidéo (ou repense à un sujet), puis résume-la en une minute. Recommence en trente secondes. Puis en dix secondes. À chaque passage, tu apprends à ne garder que l'essentiel.",
    tips: [
      "En 1 min : les idées principales. En 10 s : une seule phrase-clé.",
      "Coupe les exemples et les détails avant de couper les idées.",
      "Chronomètre-toi vraiment : tenir le temps fait partie de l'exo.",
    ],
    drillGoal:
      "Exercice de SYNTHÈSE : résumer un sujet dans un temps très contraint. Juge la capacité à aller à l'essentiel : `structure` et `clarity` priment. Le propos doit tenir dans le temps imparti sans bâcler ni déborder ; récompense la hiérarchisation (garder l'idée, couper le détail).",
  },
  {
    id: "forbidden-word",
    emoji: "🚫",
    title: "Le mot interdit",
    tagline: "Bannis un tic de langage, puis complique.",
    kind: "record",
    analysisMode: "freeSpeech",
    input: "topic",
    topicPlaceholder: "Un sujet libre à développer…",
    forbidden: true,
    forbiddenChoices: ["genre", "euh", "du coup", "voilà", "en fait", "alors"],
    description:
      "Choisis un mot que tu n'as plus le droit de dire — « genre », puis « euh », puis « du coup ». Parle sur un sujet en l'évitant totalement. Ça nettoie énormément ton langage.",
    tips: [
      "Ralentis : la plupart des tics comblent un silence de réflexion.",
      "Quand le mot te vient, remplace-le par une pause muette.",
      "Une fois un mot maîtrisé, ajoute-en un second à la liste.",
    ],
    drillGoal:
      "Exercice du MOT INTERDIT : un mot ou tic de langage précis est formellement banni. Compte ses occurrences dans la transcription et pénalise fortement `confidence` et `clarity` à chaque emploi. Félicite explicitement l'utilisateur s'il l'a évité entièrement.",
  },
  {
    id: "imitation",
    emoji: "🎙️",
    title: "Imiter un orateur",
    tagline: "Copie mot pour mot ton orateur préféré.",
    kind: "record",
    ...
    description:
      "Choisis une prise de parole que tu admires. Colle-la ci-dessous, puis rejoue-la mot pour mot en copiant sa façon de parler : son rythme, ses pauses, son intonation, son énergie. Le but est de te forcer à parler comme lui.",
    tips: [
    ...
    drillGoal:
      "Exercice d'IMITATION : l'utilisateur rejoue mot pour mot un extrait de son orateur préféré en copiant son intonation, son rythme et son énergie. Juge la fidélité au texte de référence MAIS surtout l'expressivité, la mélodie (variation d'intonation) et l'incarnation : récite-t-il platement, ou joue-t-il vraiment ? Pénalise une lecture monotone même fidèle.",
  },
  {
    id: "theatrical-reading",
    emoji: "🎭",
    title: "Lecture théâtrale",
    tagline: "Lis un texte comme un acteur.",
    kind: "record",
    analysisMode: "reading",
    input: "reference",
    referencePlaceholder: "Colle un extrait de roman, de pièce ou de poème à interpréter…",
    description:
      "Lis un passage comme si tu étais acteur : joue les émotions, varie le ton, marque les silences, incarne les personnages. C'est probablement l'exercice le plus efficace pour casser une voix monotone.",
    tips: [
    ...
    drillGoal:
      "LECTURE THÉÂTRALE : l'utilisateur lit un texte en l'interprétant comme un comédien. Juge la fidélité au texte de référence mais surtout l'expressivité, la variation d'intonation (mélodie), les silences expressifs et l'incarnation. Pénalise nettement une lecture plate ou monotone, même parfaitement fidèle.",
  },
  {
    id: "mirror",
    emoji: "🪞",
    title: "Le miroir",
    tagline: "2 minutes, les yeux dans les yeux.",
    kind: "mirror",
    timerSec: 120,
    description:
      "Deux minutes. Regarde uniquement tes propres yeux et raconte ta journée. Tu verras très vite où tu manques d'assurance — et tu apprendras à soutenir le regard.",
    tips: [
      "Ne quitte pas tes yeux du regard, même quand c'est inconfortable.",
      "Parle à voix haute, comme à quelqu'un en face de toi.",
      "Repère les moments où tu détournes les yeux : c'est là que ça flanche.",
    ],
  },
];
...
export function buildDailyAggregate(sessions, dateKey) {
  ...
  const byMode = { reading: 0, freeSpeech: 0, diction: 0, structure: 0, drills: 0 };
